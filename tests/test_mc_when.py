#!/usr/bin/env python3
"""
Test suite for Monte Carlo 'When' simulation.
...
MC_WHEN_SCRIPT = PROJECT_ROOT / "thrivve-mc-when" / "scripts" / "thrivve-mc-when.py"
...
def run_mc_when(throughput, stories_remaining, confidence_level, num_simulations, start_date):
    """Run the MC When script and return parsed JSON results."""
    cmd = [
        "python3",
        str(MC_WHEN_SCRIPT),
        throughput,
        str(stories_remaining),
    ...
    if result.returncode != 0:
        print(f"Error running MC When: {result.stderr}")
        return None
...
def main():
    """Run all tests."""
    print("=" * 60)
    print("MONTE CARLO 'WHEN' TEST SUITE")
    print("=" * 60)

    # Load test scenarios
    test_data = load_test_scenarios()
    scenarios = test_data['scenarios']
    test_config = test_data['when_tests']['basic_forecast']
